 ...
 * @param pKF 当前关键帧
 * @param Scw 世界坐标系到当前关键帧之间的相似变换
 * @param vpPoints 闭环关键帧组（闭环关键帧+与其相连的关键帧）的所有地图点，闭环地图点
 * @param vpMatched 闭环关键帧与当前关键帧之间匹配的地图点
 * @param th 搜索范围
 * @return 当前关键帧与闭环关键帧组匹配的地图点个数
 */
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    cv::Mat Ow = -Rcw.t()*tcw;// 当前关键帧光心在世界坐标系下的坐标
...
/**
 * @brief ORBmatcher::SearchForInitialization 初始化需要两帧进行匹配
 * @param F1 第一帧
 * @param F2 第二帧
 * @param vbPrevMatched 第一帧特征点坐标
 * @param vnMatches12 一个容器，保存的是按第一帧特征点的顺序，该特征点匹配上第二帧特征点的索引
 * @param windowSize 搜寻匹配是通过窗口逐层金字塔来搜索，窗口的大小
 * @return 返回的是匹配的点数
 */
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(int i=0;i<HISTO_LENGTH;i++)
        rotHist[i].reserve(500);
    const float factor = HISTO_LENGTH/360.0f;// factor=1/12
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        int level1 = kp1.octave;//代表从金字塔的哪一层取到的关键点
        // 表示只从金字塔底层寻找关键点，并搜寻匹配
        if(level1>0)
            continue;

        // 步骤一：获取在第二帧中（x，y）的位置所在的窗口中所有特征点
        // 五个参数分别为:特征点的横坐标，纵坐标，窗口大小，该特征点对应的金字塔层数索引，也是索引
        ...
        // 步骤二：遍历同一个窗口中的所有特征点，求取（x，y）特征点对应的描述符与窗口中所有特征点描述符
        // 的最短距离，次短距离，以及该描述符索引。
        // 检查是否匹配正确，要从两个角度：一描述子之间的汉明距离，匹配对的角度
        ...
        if(bestDist<=TH_LOW)// TH_LOW=50
        {
            ...
            {
                // 又匹配了该特征点一次，肯定是不对的
                if(vnMatches21[bestIdx2]>=0)// matched the KeyPoint again
                {
                    vnMatches12[vnMatches21[bestIdx2]]=-1;
                    nmatches--;
                }
                // 参考帧第i1个特征点匹配上当前帧第bestIdx2个特征点
                vnMatches12[i1]=bestIdx2;
                vnMatches21[bestIdx2]=i1;
                vMatchedDistance[bestIdx2]=bestDist;
                nmatches++;

                // rotHist数据结构：长度为30（将两个特征点之间的夹角0°~360°，每12°均分为30份）
                // 只要在第一帧中的某个特征点在第二帧中找到描述子在距离上的匹配，那么这两点之间就会
                // 有角度差，根据这个差值等于这30份里的某份（rotHist[i])，
                // 那么rotHist[i]存储的是第一帧特征点的索引。
                if(mbCheckOrientation)
                {
                                       ...
                                       vector<pair<size_t, size_t> > &vMatchedPairs)
{
    ...
    cv::Mat C2 = R2w*Cw+t2w; // tc2c1 KF1的相机中心在KF2 camera坐标系的表示
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for(int i=0; i<nMPs; i++)
    {
        ...
        const float minDistance = pMP->GetMinDistanceInvariance();
        cv::Mat PO = p3Dw-Ow;
        const float dist3D = cv::norm(PO);//对向量单位化

        // Depth must be inside the scale pyramid of the image
        if(dist3D<minDistance || dist3D>maxDistance )
            continue;

        // Viewing angle must be less than 60 deg，cos60=0.5
        cv::Mat Pn = pMP->GetNormal();

        if(PO.dot(Pn)<0.5*dist3D)// dot()函数为点积运算，对应位置相乘再相加求和，最后得到一个数
            continue;

        int nPredictedLevel = pMP->PredictScale(dist3D,pKF);//预测当前帧在图像金字塔第几层
