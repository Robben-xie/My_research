...
void MapDrawer::DrawMyMesh()
{
    const vector<Triangle*> list_face = mpMap->GetAllMeshFaces();
    ...
    glBegin(GL_LINES);
    for(size_t i=0; i<list_face.size(); i++)
    {
        cv::Point3f v1,v2,v3;
        Triangle* Tri = list_face[i];
        v1 = Tri->getV0();
        v2 = Tri->getV1();
        v3 = Tri->getV2();
