...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            ...
            pFrame->mvbOutlier[i] = false;// 该点设置为内点
    ...
    pFrame->SetPose(pose);// pose

    return nInitialCorrespondences-nBad;
}

int Optimizer::PoseBundleAdjustment(Frame* pFrame, cv::Mat &Pose, std::vector<int> &vIndex, std::vector<cv::Point3f> &list3D)
{
    // 该优化函数主要用于Tracking线程中：参考帧跟踪
...
/*
void Optimizer::PoseBundleAdjustment(Frame* pFrame, cv::Mat &Pose, std::vector<int> &vIndex, std::vector<cv::Point3f> &list3D)
{
    // 步骤1：构造g2o优化器
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    for(size_t i = 0; i < vIndex.size(); i++)
    {
        if(vIndex[i]<0)
            continue;
        g2o::VertexSBAPointXYZ* vPoint = new g2o::VertexSBAPointXYZ();
        vPoint->setEstimate(Converter::toVector3d(list3D[i]));
        vPoint->setId(index);
        vPoint->setMarginalized(true);
        optimizer.addVertex(vPoint);
        ...
        //SET EDGES
        // 步骤3：向优化器添加投影边边
        const cv::KeyPoint &kpUn = pFrame->mvKeysUn[vIndex[i]];
    ...
    //Keyframes
    g2o::VertexSE3Expmap* vpose = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(0));
    g2o::SE3Quat SE3quat_recov = vpose->estimate();
    Pose = Converter::toCvMat(SE3quat_recov);
    ...
    for(size_t i=0; i<vIndex.size(); i++)
    {
        ...
        cv::Mat P3D = Converter::toCvMat(vPoint->estimate());
        list3D[i].x = P3D.at<float>(0);
        list3D[i].y = P3D.at<float>(1);
        list3D[i].z = P3D.at<float>(2);
        index++;
    }
}
*/
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2)
{
    ...
    vSim3->setId(0);
    vSim3->setFixed(false);// 优化Sim3顶点, false means cannot fix during the optimization
