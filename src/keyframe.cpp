...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy), N(F.N),
...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    cv::Mat Rwc = Rcw.t();
    Ow = -Rwc*tcw;// the world coordinate
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    // 按照权重进行排序
    std::sort(vPairs.begin(),vPairs.end(),compare);
...
{
    unique_lock<mutex> lock(mMutexConnections);
    return mvpOrderedConnectedKeyFrames;//连接已按权值排列的关键帧
}
...
float KeyFrame::ComputeSceneMedianDepth(const int q)
{
    ...
    sort(vDepths.begin(),vDepths.end());//先排序

    return vDepths[(vDepths.size()-1)/q];//再求中值
}
