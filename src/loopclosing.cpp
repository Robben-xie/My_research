...
void LoopClosing::InsertKeyFrame(KeyFrame *pKF)
{
    ...
    if(pKF->mnId!=0)//如果关键帧不为第一帧,则将关键帧堆入队列中
...
{
    {
        // 从队列中取出一个关键帧
        unique_lock<mutex> lock(mMutexLoopQueue);
        mpCurrentKF = mlpLoopKeyFrameQueue.front();//返回队列中起始元素的引用(取出第一个关键帧)
        mlpLoopKeyFrameQueue.pop_front();//删除队列开头的元素
        // Avoid that a keyframe can be erased while it is being process by this thread
        mpCurrentKF->SetNotErase();//避免关键帧被擦除当它正在被线程处理时，设置标志位
    }
    ...
    {
        mpKeyFrameDB->add(mpCurrentKF);//增加关键帧(类型为KeyFrameDatabase*)
    ...
    float minScore = 1;// 令最小得分为1
    for(size_t i=0; i<vpConnectedKeyFrames.size(); i++)//遍历所有共视帧
    {
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nInitialCandidates; i++)
        {
            ...
            if(!Scm.empty())
            {
                ...
                const int nInliers = Optimizer::OptimizeSim3(mpCurrentKF, pKF, vpMapPointMatches, gScm, 10);// 卡方chi2检验阈值
