...
cv::Mat MapPoint::GetNormal()//返回世界坐标系下，3D点到相机关心的向量
{
    unique_lock<mutex> lock(mMutexPos);
...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    if(mObservations.count(pKF))
        return;//mObservtions中有PKF，则返回，否则执行下面索引
...
/**
 * @brief MapPoint::PredictScale predict Scale
 * @param currentDist         currentframe's scale
 * @param pF
 * @return  currentframe's keypoint at which level
 */
